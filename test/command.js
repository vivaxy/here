...
describe('test terminal command `here`', () => {
  ...
  it(`\`here\` should output \`${logPrefix.SERVER} listen http://*.*.*.*:*/\``, (done) => {
    ...
    here.stdout.on('data', (data) => {
      data = data.toString();
      let regExp = `${logPrefix.SERVER} listen http:\\/\\/\\d+\\.\\d+\\.\\d+\\.\\d+:\\d+\\/\\n`;
  ...
  it(`\`here -w\` should output \`${logPrefix.SERVER} listen http://*.*.*.*:*/\` and \`${logPrefix.WATCH} ready, reload in 0 seconds\``, (done) => {
    ...
    here.stdout.on('data', (data) => {
      ...
      switch (stdoutCount) {
        case 1:
          let serverRegExp = `${logPrefix.SERVER} listen http:\\/\\/\\d+\\.\\d+\\.\\d+\\.\\d+:\\d+\\/\\n$`;
          assert.equal(true, new RegExp(serverRegExp).test(data));
          break;
        case 2:
          let watchRegExp = `${logPrefix.WATCH} ready, reload in 0 seconds\\n$`;
    ...
    here.stdout.on('data', (data) => {
      ...
      switch (stdOutCount) {
        case 1:
          ...
          break;
        case 2:
          let watchRegExp = `${logPrefix.WATCH} ready, reload in 3 seconds\\n$`;
          assert.equal(true, new RegExp(watchRegExp).test(data));
          here.kill();
          done();
          break;
