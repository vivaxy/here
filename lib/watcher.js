...
p.init = function () {
    ...
    var WebSocketServer = ws.Server,
        wss = new WebSocketServer({port: _this.port}, function () {
            _this.log.info('watcher: listen http://' + hostname + ':' + _this.port + '/');
    ...
    wss.on('error', function (err) {
        if (err.code === 'EADDRINUSE') {
            _this.log.warn('watcher: port ' + _this.port + ' in use');
...
p.listen = function () {
    var _this = this,
        reload = debounce(function () {
            // reload
            _this.log.debug('watcher: reload');
    ...
    return chokidar.watch(currentDir, {ignored: this.getIgnored.bind(this)}).on('all', function (action, filename) {
        ...
        _this.log.debug('watcher: ' + _this.toDashedString(action) + ' ' + relativeFilename);
