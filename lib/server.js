...
p.initialize = function () {
    ...
    this.server = http.createServer(function (req, res) {
        ...
        _this.log && log('server : ' + chalk.cyan(req.method) + ' ' + reqUrl);
        ...
        fs.readFile(responseFile, function (err, data) {
            if (err) {
                ...
                // request a file
                _this.respondFile(res, pathname, mime.lookup(extension) || 'text/plain', data);
            }
        });
    });
};
...
p.startup = function () {
    ...
    this.server.on('error', function (err) {
        if (err.code === 'EADDRINUSE') {
            // red
            log('server : ' + chalk.red('port ' + _this.port + ' in use'));
    ...
    this.server.listen(this.port, function () {
        ...
        // green
        log('server : ' + chalk.cyan('LISTEN ') + openUrl);
