...
const prepare = () => {
  ...
  commander
    ...
    .option(`-S, --${configKeys.SSL}`, 'switch protocol to https')
    .option(`-G, --${configKeys.GZIP}`, 'open gzip')
    .option(
    ...
    .option(`-s, --${configKeys.SILENT}`, 'will not open browser')
    .option(`--ip [ip]`, 'open browser with ip', config.get(configKeys.IP))
    .option(
  ...
  if (commanderOptions[configKeys.IP]) {
    config.set(configKeys.IP, commanderOptions[configKeys.IP]);
  }
