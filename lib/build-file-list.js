...
module.exports = (files, pathname, absoluteWorkingDirectory) => {
    ...
    let filesFiltered = files.filter((file) => { // eslint-disable-line newline-after-var
        ...
        try {
            ...
            if (isWindows) {
                ...
                fs.statSync(join(baseDir, file)); // eslint-disable-line no-sync

            } else {

                fs.accessSync(join(baseDir, file), fs.R_OK); // eslint-disable-line no-sync
    ...
    filesFiltered.sort((a, b) => { // eslint-disable-line max-statements

        let _isDirectoryA = fs.lstatSync(join(baseDir, a)).isDirectory(); // eslint-disable-line no-sync
        let _isDirectoryB = fs.lstatSync(join(baseDir, b)).isDirectory(); // eslint-disable-line no-sync
    ...
    let list = filesFiltered.map((file) => {
        ...
        if (fs.lstatSync(join(baseDir, file)).isDirectory()) { // eslint-disable-line no-sync
